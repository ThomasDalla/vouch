...
func TestObtainPayloadTimings(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                "served values",
			spec:                map[string]any{"PAYLOAD_DUE_BPS": uint64(5000), "PAYLOAD_ATTESTATION_DUE_BPS": uint64(7000)},
			expectedPayloadDue:  6 * time.Second,
			expectedAttestation: 8400 * time.Millisecond,
		},
		{
			name:                "fork-suffixed values take precedence",
			spec:                map[string]any{"PAYLOAD_DUE_BPS": uint64(5000), "PAYLOAD_DUE_BPS_GLOAS": uint64(2500)},
